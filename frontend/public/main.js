...
function loadComponent(className, componentPath) {
  fetch(componentPath)
    .then(response => {
      if (!response.ok) {
        throw new Error(`Error al cargar ${componentPath}: ${response.status} ${response.statusText}`);
      }
    ...
    .catch(error => console.error(`Error al cargar el componente: ${error.message}`));
}
